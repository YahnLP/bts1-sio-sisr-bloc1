# 05 – TP1 : Objectifs et ressources

Ce premier TP de l'année met en pratique les notions de **dissolution** (S02), **concentration** (S03) et **dilution** (S04). C'est une séance fondamentale pour acquérir les gestes de laboratoire.

---

## 🎯 Objectifs

### Objectifs scientifiques

* Préparer une **solution mère par dissolution** (pesée + ajustement)
* Réaliser des **dilutions** pour obtenir une gamme de concentrations
* Utiliser une **échelle de teinte** pour estimer une concentration inconnue
* Vérifier la **conformité** d'un produit par rapport à un cahier des charges

### Objectifs méthodologiques (E2)

* **Manipuler** avec rigueur (pesée, pipetage, ajustement)
* **Analyser** : exploiter une gamme étalon
* **Communiquer** : rédiger un compte-rendu de TP structuré

---

## 🧠 Compétences travaillées

* **Organiser** : préparer son poste, anticiper les étapes
* **Manipuler** : utiliser la verrerie jaugée avec précision
* **Analyser** : comparer, estimer, conclure
* **Communiquer** : rédiger un bilan de TP

---

## 📥 Ressources à télécharger

### 📘 Documents élève

➡️ [Télécharger la fiche de TP (PDF)](05a_fiche_eleve.pdf)

➡️ [Télécharger la trace écrite – Bilan du TP (PDF)](05b_trace_ecrite.pdf)

### 🔧 Fiches méthode associées

➡️ [Fiche méthode 02 – Calculer et interpréter une concentration](../Methodologie/02_fiche_methode/)

➡️ [Fiche méthode 03 – Exploiter un TP à l'écrit](../Methodologie/03_fiche_methode/)

➡️ [Fiche méthode 04 – Choisir et justifier une dilution](../Methodologie/04_fiche_methode/)

---

## 📺 Pour préparer le TP (à regarder AVANT)

🎬 [Utiliser une pipette jaugée](https://www.youtube.com/watch?v=0rSYpJfPJfE) – 3 min

🎬 [Préparer une solution par dissolution](https://www.youtube.com/watch?v=vqX9QXMJ8bQ) – 4 min

🎬 [Réaliser une dilution](https://www.youtube.com/watch?v=K6KLg_R0GKE) – 4 min

💡 **Conseil** : Regardez ces vidéos la veille du TP pour arriver préparé(e) !

---

## ⚠️ Consignes de sécurité

- 🥽 **Lunettes de protection** obligatoires
- 🧤 **Gants** recommandés (le colorant tache)
- 🧥 **Blouse** fermée

---

## 🔗 Navigation

⬅️ Séance précédente : [S04 – Dilution](../04_Dilution/)

➡️ Séance suivante : [S06 – E2 guidé n°1](../06_Evaluation1/)
